...
from sklearn.decomposition import FactorAnalysis as DR
import numpy as np
...
start = time.time()
DR1 = DR(n_components=10)
x_train1 = DR1.fit_transform(x_train)
x_test1 = DR1.transform(x_test)
end = time.time()
alltime.append(end-start)

start = time.time()
DR2 = DR(n_components=20)
x_train2 = DR2.fit_transform(x_train)
x_test2 = DR2.transform(x_test)
end = time.time()
alltime.append(end-start)

start = time.time()
DR3 = DR(n_components=30)
x_train3 = DR3.fit_transform(x_train)
x_test3 = DR3.transform(x_test)
end = time.time()
alltime.append(end-start)

start = time.time()
DR4 = DR(n_components=40)
x_train4 = DR4.fit_transform(x_train)
x_test4 = DR4.transform(x_test)
end = time.time()
alltime.append(end-start)

start = time.time()
DR5 = DR(n_components=50)
x_train5 = DR5.fit_transform(x_train)
x_test5 = DR5.transform(x_test)
end = time.time()
alltime.append(end-start)

start = time.time()
DR6 = DR(n_components=100)
x_train6 = DR6.fit_transform(x_train)
x_test6 = DR6.transform(x_test)
end = time.time()
alltime.append(end-start)
...
plt.title("Model Accuracy with Factor Analysis")
plt.ylabel("Accuracy")
plt.xlabel("Epoch")
